/**
 * File: GanttChartView.cpp
 * Description: The implementation of the GanttChartView class.
...
namespace hbchart {
    ...
    GanttChartView::GanttChartView(QGraphicsScene* scene, QWidget* parent)
        ...
        connect(m_debouceTimer, &QTimer::timeout, this, &GanttChartView::onApplyPendingScale);
    }

    void GanttChartView::setZoom(qreal wzoom, qreal hzoom) {
        ...
        scale(zoom, zoom);
        //
        auto* ganttScene = qobject_cast<GanttChartScene*>(scene());
        if (ganttScene != nullptr) {
            ganttScene->updateViewScaleFactor(zoom);
        }
    }
    ...
    void GanttChartView::mouseReleaseEvent(QMouseEvent* event) {
    ...
    void GanttChartView::onApplyPendingScale() {
        // Apply delay scaling
        auto* ganttScene = qobject_cast<GanttChartScene*>(scene());
        if (ganttScene != nullptr) {
            ganttScene->updateViewScaleFactor(m_pendingScale);
        }
